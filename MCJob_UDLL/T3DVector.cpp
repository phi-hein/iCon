...
void T3DVector::Transform2Jump(T3DVector& inp_vec, const T3DVector& jump_vec, const T3DVector& norm_vec)
{
	...
	if ((norm_vec.Length() < zero_threshold) || (abs(y) < zero_threshold))
	{	// System mit nur Atomen auf Sprungachse bzw. Atom auf Sprungachse
		y = 0;
		z = 0;
	}
	else
	{
		if (Normalvector(inp_vec, jump_vec) * Cross(norm_vec, jump_vec) >= 0)
		{  // hier auch noch Grenzen der Funktionen beachten
