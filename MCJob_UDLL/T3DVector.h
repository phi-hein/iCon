...
// Klassendeklaration:
class T3DVector {
		friend ostream & operator << (ostream &out, const T3DVector &param) {		// Vektorausgabe
	...
	public:
		...
		// Member functions:
		T3DVector () : x(0.0), y(0.0), z(0.0) {						// Default-Constructor
		}
		T3DVector (double set_x,double set_y,double set_z) :		// Constructor mit Initialisierung
			x(set_x), y(set_y), z(set_z) {
		}
		void Set (double set_x,double set_y,double set_z) {			// Initialisierung
			x = set_x; y = set_y; z = set_z;
		}
		double Length () const {									// Vektorbetrag
			return sqrt(x*x+y*y+z*z);
		}
		T3DVector operator + (const T3DVector &param) const {		// Vektoraddition
			return T3DVector(x + param.x,y + param.y,z + param.z);
		}
		T3DVector& operator += (const T3DVector &param) {			// Vektoraddition
			x += param.x;
			y += param.y;
			z += param.z;
			return *this;
		}
		T3DVector operator - (const T3DVector &param) const {		// Vektorsubtraktion
			return T3DVector(x - param.x,y - param.y,z - param.z);
		}
		T3DVector operator * (double param) const {					// Multiplikation mit Skalar
			return T3DVector(x*param,y*param,z*param);
		}
		double operator * (const T3DVector &param) const {			// Skalarprodukt
			return x*param.x + y*param.y + z*param.z;
		}
		bool operator == (const T3DVector &param) const {			// Vectorvergleich mit Threshold
			if ((abs(x-param.x) <= eq_threshold) &&
				(abs(y-param.y) <= eq_threshold) &&
				(abs(z-param.z) <= eq_threshold)) return true;
			return false;
		}
		bool operator != (const T3DVector &param) const {			// Vectorungleich mit Threshold
			if (*this == param) return false;
			return true;
		}
		bool operator > (const T3DVector &param) const {			// Groesser-Operator mit Threshold
